...
pub(super) fn event_requires_state_sync(event: &Event) -> bool {
    matches!(event, Event::Exec { .. } | Event::SaveGame { .. })
}
            ...
            match event {
                ...
                Event::LoadGame { file, trans_type } => {
                    ...
                    if file.is_empty() {
                        ...
                        // Load replaces the interpreter position and scene. The rest of this
                        // already-drained batch belongs to the old save/load UI and must not be
                        // applied over the restored state. Events generated by onLoad remain in
                        // the shared queue and are dispatched on the next tick.
                        break;
                    }
                }
                Event::GoTitle => {
...
mod tests {
    use super::{
        crc32_ieee, layer_info_entry, pointer_property_invalidates_hit_test, sync_tween_finished,
        sync_tween_wait_layer, sync_tween_wait_reason,
    };
