...
impl TextureProvider for MetalBackend {
    fn resolve(&mut self, name: &str) -> Option<(TextureId, TextureInfo)> {
        ...
        if crate::naming::is_video_layer_texture_name(name) {
            return None;
        }
    ...
    fn solid_texture(&mut self, rgba: [u8; 4]) -> Option<(TextureId, TextureInfo)> {
        let name = crate::draw::solid_texture_name(rgba);
        if let Some(&id) = self.names.get(&name) {
            return Some((
                id,
                TextureInfo {
    ...
    fn resolve_with_mask(&mut self, file: &str, mask: &str) -> Option<(TextureId, TextureInfo)> {
        let name = crate::draw::masked_texture_name(file, mask);
        if let Some(&id) = self.names.get(&name) {
            return self.textures.get(&id).map(|texture| (id, texture.info));
        }
...
impl GpuBackend for MetalBackend {
    ...
    fn configure_post_process(&mut self, pipeline: PostProcessPipeline) -> Result<(), String> {
        ...
        if spatial_requested && !self.metalfx_supported {
            ...
            log::info!("[MetalFX] spatial requested but unavailable; native render selected");
            return Ok(());
        }
    ...
    fn clear(&mut self, color: [f32; 4]) {
        let Some(frame) = self.active_frame.as_ref() else {
            log::warn!("[MetalBackend] clear called without an active frame");
            return;
        };
        if let Err(error) = encode_clear(&frame.command_buffer, &frame.target, color) {
            log::warn!("[MetalBackend] clear failed: {error}");
        }
    }
    ...
    fn acquire_video_surface(
        ...
        if let Some(&id) = self.names.get(name)
            ...
            && let Some(external) = stored.external.as_ref()
        {
    ...
    fn clear_native_surface(&mut self) {
        ...
        {
            log::warn!("[MetalFX] failed to restore scene target: {error}");
        }
    }
    ...
    fn set_profile_enabled(&self, enabled: bool) {
        ...
        if !enabled {
            let _ = self.drain_profile_stats();
        }
    }

    fn take_profile_stats(&mut self) -> GpuProfileStats {
        self.drain_profile_stats()
    }
}

impl MetalBackend {
    fn drain_profile_stats(&self) -> GpuProfileStats {
    ...
    fn render_internal(
        ...
        let Some(active) = self.active_frame.as_ref() else {
            log::warn!("[MetalBackend] render called without an active frame");
            return repaint;
        };
        ...
        if let Err(error) = result {
            log::warn!("[MetalBackend] render encoding failed: {error}");
        }
    ...
    fn execute_post_process(
        ...
        if spatial && spatial_has_work {
            ...
            log::warn!("[MetalFX] scaler creation failed; using linear present");
        }
...
fn shader_group_blend(group: &ShaderGroup) -> BlendMode {
    if group.effect.name != crate::shader::GROUP_COMPOSITE_SHADER {
...
mod tests {
    ...
    fn native_metal_renders_sprite_mesh_and_offscreen_target() {
        ...
        mesh_command.mesh = Some(crate::draw::DrawMesh {
            vertices: vec![
                [0.0, 0.0, 0.0, 0.0],
                [4.0, 0.0, 1.0, 0.0],
                [4.0, 4.0, 1.0, 1.0],
    ...
    fn native_metal_executes_stencil_mask_group() {
        ...
        frame.shader_groups.push(ShaderGroup {
            ...
            effect: ShaderEffect {
                name: crate::shader::ALPHA_MASK_SHADER.to_owned(),
    ...
    fn native_metal_group_composite_applies_group_alpha() {
        ...
        frame.shader_groups.push(ShaderGroup {
            ...
            effect: ShaderEffect {
                name: crate::shader::GROUP_COMPOSITE_SHADER.to_owned(),
                uniforms: [
    ...
    fn metalfx_present_writes_one_complete_upscaled_frame() {
        ...
        pipeline.passes[0] = PostProcessPass::Upscale(crate::post_process::UpscaleConfig {
    ...
    fn metal_uploads_video_rgba_without_placeholder() {
        ...
        let name = crate::naming::video_layer_texture_name("movie");
    ...
    #[test]
    fn metal_uploads_video_yuv420p_planes() {
        let Ok(mut backend) = MetalBackend::new(2, 2) else {
            return;
        };
        let name = crate::naming::video_layer_texture_name("movie-yuv");
        assert!(backend.supports_video_yuv420p());
        assert!(backend.upload_video_yuv420p(
            &name,
            2,
            2,
            Yuv420pPlanes {
        ...
        let expected = backend.names[&name];
        assert_eq!(
    ...
    fn metal_imports_mtltexture_zero_copy_and_tracks_consumption() {
        ...
        let name = crate::naming::video_layer_texture_name("layer");
        let handle = backend
