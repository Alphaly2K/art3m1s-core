...
use crate::backend::*;
use crate::draw::*;
use crate::hlsl::{CompiledShader, ShaderCompiler, ShaderRegistry, ShaderRuntimeParameters};
use crate::post_process::{PostProcessPipeline, RenderDimensions};
...
impl VulkanBackend {
    ...
    fn update_image(&mut self, id: TextureId, update: TextureUpdate<'_>) -> Result<(), String> {
        ...
        ) && (!update.origin[0].is_multiple_of(4)
            || !update.origin[1].is_multiple_of(4)
            || (!update.extent.width.is_multiple_of(4) && end[0] != t.image.desc.extent.width)
            || (!update.extent.height.is_multiple_of(4) && end[1] != t.image.desc.extent.height))
        {
...
impl GpuBackend for VulkanBackend {
    ...
    fn clear_native_surface(&mut self) {
        ...
        {
            log::warn!("[VulkanBackend] failed to restore scene target: {error}");
        }
    }
...
impl VulkanBackend {
    ...
    fn collect(&mut self) {
        while let Some(s) = self.submissions.front() {
