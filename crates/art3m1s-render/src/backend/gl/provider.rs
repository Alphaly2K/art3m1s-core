...
//! 与解码/上传解耦：宿主可以注入项目目录读取、内存读 `.pfs` 或其他素材来源，
//! provider 这边无需改动。默认无字节源、一律回退占位，让整条绘制管线无需素材
//! 即可端到端验证。

use super::{ShaderProfile, platform, shader};
use crate::backend::{
    AssetSource, Extent2D, RenderTargetId, TextureData, TextureUpdate, Yuv420pPlanes,
};
use crate::draw::{TextureId, TextureInfo, TextureProvider};
use glow::HasContext;
use std::cell::Cell;
    ...
    unsafe fn try_create_texture(
        ...
        unsafe {
            let tex = match gl.create_texture() {
                Ok(tex) => tex,
                Err(e) => {
                    log::warn!("create_texture 失败 ({width}x{height}): {e}");
                    return None;
                }
            };
        ...
        if let Some(source) = &self.source {
            match source(name) {
                Some(bytes) => match decode_rgba(&bytes) {
                    ...
                    None => {
                        log::warn!("纹理解码失败，回退占位: {name}");
                    }
                },
                None => {
                    log::warn!("素材不存在，回退占位: {name}");
                }
            }
        }
