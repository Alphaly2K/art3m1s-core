...
static ANGLE_PATH: OnceLock<PathBuf> = OnceLock::new();

/// Configures the directory used to resolve bundled ANGLE libraries.
///
/// The host should call this before creating an ANGLE-backed GL context. The
/// first non-empty path wins, matching the existing host callback semantics.
pub fn set_angle_path_prefix(prefix: impl Into<PathBuf>) -> bool {
    let prefix = prefix.into();
    if prefix.as_os_str().is_empty() {
        return false;
    }
    ANGLE_PATH.set(prefix).is_ok()
}

fn angle_lib_path(name: &str) -> String {
    ANGLE_PATH
        .get()
        .map(|prefix| prefix.join(name).to_string_lossy().into_owned())
...
type CreatedPlatformContext = (Rc<glow::Context>, Box<dyn GLPlatformContext>, GfxBackend);
...
pub fn create_offscreen_context(
    ...
    stage_w: u32,
    stage_h: u32,
) -> Result<CreatedPlatformContext, String> {
    match backend {
        ...
        GfxBackend::Angle(sub) => match create_egl(sub, stage_w, stage_h) {
            ...
            Err(e) => {
                ...
                log::warn!("ANGLE failed ({e}), falling back to CGL");
        ...
        pub fn make(
            ...
            unsafe {
                ...
                let egl_candidates = if cfg!(target_os = "ios") {
                    vec![angle_lib_path("libEGL.framework/libEGL")]
                    ...
                    vec![
                        angle_lib_path("libEGL.framework/libEGL"),
                        angle_lib_path("libEGL.dylib"),
                    ]
                    ...
                    vec!["libEGL.so.1".to_string(), angle_lib_path("libEGL.so")]
                } else {
                    vec![angle_lib_path("libEGL.dll")]
                };
                ...
                let gles_candidates = if cfg!(target_os = "ios") {
                    vec![angle_lib_path("libGLESv2.framework/libGLESv2")]
                } else if cfg!(target_os = "macos") {
                    vec![
                        angle_lib_path("libGLESv2.framework/libGLESv2"),
                        angle_lib_path("libGLESv2.dylib"),
                    ]
                } else if cfg!(target_os = "android") {
                    ...
                    vec!["libGLESv2.so.2".to_string(), angle_lib_path("libGLESv2.so")]
                } else {
                    vec![angle_lib_path("libGLESv2.dll")]
                };
...
/// Creates an RGBA8 texture and framebuffer target.
///
/// # Safety
/// `gl` must be a current, valid GL context for the duration of the call.
...
/// Reads RGBA8 pixels from the currently bound framebuffer.
///
/// # Safety
/// `gl` must be a current, valid GL context for the duration of the call.
