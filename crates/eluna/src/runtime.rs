...
impl ElunaPlayer {
    ...
    pub fn set_stereovision_screen_index(&mut self, screen_index: usize) {
        self.stereovision_screen_index =
            screen_index.min(self.stereovision_screen_count.saturating_sub(1));
        self.modified = true;
    }
    ...
    pub fn set_variable_immediate(&mut self, name: &str, value: f32) {
        if self.set_control_variable(name, value, 0.0, 0.0).is_some() {
            self.pending_writes
        ...
        for (index, control) in self.runtime_pipeline.transition_controls.iter().enumerate() {
            if control.label == name {
                if let Some(state) = self.transition_states.get_mut(index) {
                    set_scalar_transition_target(state, value, time_ticks, easing, self.queuing);
                    return Some(value);
                }
            }
        }
    ...
    fn issue_timeline_frame_command(
        ...
        if difference_path {
            ...
            let _ =
                self.set_variable_timed_internal(&variable.name, target, duration, frame.easing);
        }
    }
    ...
    fn seek_active_timeline(&mut self, name: &str, time_ticks: f32) {
        ...
        for (var_index, variable) in timeline.variables.iter().enumerate() {
            ...
            if let Some(index) = latest_command {
                self.issue_timeline_frame_command(name, variable, index, target_time, mode);
            }
        }
    ...
    fn advance_active_timeline_to(&mut self, name: &str, target_time: f32, inclusive: bool) {
        ...
        for (var_index, variable) in timeline.variables.iter().enumerate() {
            ...
            loop {
                ...
                cursor = next_index;
                self.issue_timeline_frame_command(name, variable, cursor, target_time, mode);
            }
            state.frame_indices[var_index] = cursor;
        }
        state.elapsed_ticks = target_time;
    ...
    fn progress_active_timelines(&mut self, delta_ticks: f32) {
        ...
        for name in names {
            ...
            let stop_on_idle = self
                .active_timeline_states
                .get(&name)
                .map(|state| state.stop_when_blend_idle && timeline_blend_transition_idle(state))
                .unwrap_or(false);
    ...
    fn evaluate_runtime_control_step(&mut self, pipeline: &EmoteRuntimePipeline, step_ticks: f32) {
    ...
    fn progress_difference_timeline_targets_for(&mut self, timeline_name: &str, delta_ticks: f32) {
        ...
        for state in variables.values_mut() {
            ...
            state.value = target.start_value + (target.target_value - target.start_value) * eased;
            if t < 1.0 {
        ...
        while wind.spawn_accumulator >= 0.0 {
            ...
            if rand::random::<f32>() < 0.0625 {
                if let Some(pulse) = wind.pulses.iter_mut().find(|pulse| !pulse.active) {
                    ...
                    pulse.power =
                        wind.pow_min + (wind.pow_max - wind.pow_min) * rand::random::<f32>();
                }
            }
        }
...
impl EmotePlayerControl for ElunaPlayer {
    ...
    fn set_outer_force(&mut self, label: &str, x: f32, y: f32, time_ticks: f32, easing: f32) {
        if !matches!(label, "bust" | "hair" | "parts") || !x.is_finite() || !y.is_finite() {
            return;
        }
        ...
        if let Some(state) = self.outer_force_states.get_mut(label) {
            set_vector2_transition_target(state, [x, y], duration, easing, self.queuing);
        }
        self.modified = true;
        self.record_api_call(
            "SetOuterForce",
    ...
    fn set_outer_rot(&mut self, rot: f32, time_ticks: f32, easing: f32) {
        ...
        self.record_api_call(
            "SetOuterRot",
            vec![rot.to_string(), duration.to_string(), easing.to_string()],
        );
    }
...
fn advance_timeline_blend(state: &mut ActiveTimelineState, delta_ticks: f32) -> f32 {
    ...
    loop {
        ...
        // the output to [0,1].
        state.blend_ratio = target.start_value + (target.target_value - target.start_value) * eased;
        if t < 1.0 - 0.0001 {
...
fn step_vector2_transition(state: &mut Vector2TransitionState, delta_ticks: f32) {
    ...
    loop {
        ...
        let eased = state.progress.clamp(0.0, 1.0).powf(state.easing_exponent);
        state.current = [
...
fn augment_variable_ranges_from_controls(
    ...
    for control in &pipeline.eye_controls {
        let state =
            ensure_variable_range_entry(variables, &control.label, control.begin_frame as f32);
    ...
    for control in &pipeline.selector_controls {
        ...
        merge_range_value(
            &mut state.info.min_value,
            &mut state.info.max_value,
            max_index,
        );
        for option in &control.option_list {
            ...
            let transition =
                ensure_variable_range_entry(variables, &option.label, option.off_value);
            merge_range_value(
...
pub fn collect_emote_runtime_pipeline(psb: &PsbFile) -> EmoteRuntimePipeline {
    ...
    pipeline.stereovision_control =
        parse_stereovision_control(metadata.field("stereovisionControl"));
...
fn parse_graph_edges(value: Option<&PsbValue>) -> Vec<[f32; 2]> {
    value
        ...
        .filter_map(|edge| {
            ...
            Some([items.first()?.as_f32()?, items.get(1)?.as_f32()?])
        })
        .collect()
}
...
fn graph_find_line(edges: &[[f32; 2]], value: f32) -> Option<usize> {
    edges
        .iter()
        .position(|edge| graph_line_contains(edge, value))
}
    ...
    for _ in 0..guard {
        match state.stage {
            ...
            2 => {
                ...
                let eased_distance_fraction = (delta_ticks * state.inv_duration
                    + old_time_fraction)
                ...
                if reached {
                    let correction = (state.segment_target - state.current) * state.direction;
        ...
        match state.blink_state {
            ...
            EyeBlinkState::ClosedHold => {
                ...
                if state.blink_timer <= 0.0 {
                    ...
                    state.blink_timer =
                        native_random_range(control.blink_interval_min, control.blink_interval_max);
                    continue;
                }
            }
...
fn stereovision_variable_is_targeted(pipeline: &EmoteRuntimePipeline, variable_name: &str) -> bool {
...
fn stereovision_screen_for_range(
    ...
    let p = fov * level;
    let anchor_ratio = (screen_count - vector_index - 1) as f32 / (screen_count - 1) as f32;
...
fn apply_stereovision_screen_projection(
    ...
    let Some(vector_index) =
        stereovision_vector_index(physical_screen_index, screen_count, reverse_screens)
    else {
        return;
    };
    ...
    for (name, state) in variables.iter_mut() {
        ...
        let Some(screen) =
            stereovision_screen_for_range(min, max, vector_index, screen_count, level, fov)
        else {
            continue;
        };
...
fn step_hair_physics_once(
    ...
    let var_ud = physics_output_response((state.ofs - ud_disp) * scale_y[ud_eft] * output_scale);
...
fn collect_timeline_nodes(value: &PsbValue, out: &mut Vec<EmoteTimeline>) {
    match value {
        ...
        PsbValue::Object(_) => {
            let Some(variable_list) = value.field("variableList").and_then(PsbValue::as_list)
            else {
            ...
            for variable in variable_list {
                let Some(var_name) = variable.field_str("label").filter(|name| !name.is_empty())
                else {
                    continue;
                };
...
fn collect_runtime_control_variable_infos(
    ...
    for control in &pipeline.eye_controls {
        ...
        merge_runtime_variable_ref(out, &control.label, control.begin_frame as f32, min, max);
    }
    for control in &pipeline.eyebrow_controls {
        let (min, max) = graph_control_range(
            control.begin_frame as f32,
    ...
    for control in &pipeline.clamp_controls {
        ...
        for name in [&control.var_lr, &control.var_ud] {
            merge_runtime_variable_ref(out, name, 0.0, Some(control.min), Some(control.max));
        }
    }
    for control in &pipeline.physics_controls {
        let def = match control {
            PhysicsControl::Bust(def) | PhysicsControl::Hair(def) | PhysicsControl::Parts(def) => {
                def
            }
        };
        if !def.enabled {
...
mod reverse_parity_tests {
    ...
    fn stereovision_two_screen_coefficients_match_native_builder() {
        ...
        let left = stereovision_screen_for_range(-10.0, 10.0, 0, 2, 1.0, 0.2).expect("screen 0");
        let right = stereovision_screen_for_range(-10.0, 10.0, 1, 2, 1.0, 0.2).expect("screen 1");
    ...
    fn stereovision_projection_only_changes_resolved_targets() {
        ...
        apply_stereovision_screen_projection(&pipeline, &mut values, 0, 2, 1.0, 0.2, false);
    ...
    fn difference_overlay_runs_before_loop_control_overwrite() {
        ...
        assert_eq!(
            player.evaluated_variable_values().get("loop_pose").copied(),
            Some(7.0)
        );
    }
    ...
    fn physics_output_is_published_after_mirror_and_clamp_stage() {
        ...
        assert_eq!(
            player.evaluated_variable_values().get("hair_lr").copied(),
            Some(2.0)
        );
    }
