...
impl EmoteMeshPatch {
    ...
    pub fn combined_with(&self, next: &EmoteMeshPatch) -> EmoteMeshPatch {
        ...
        for i in 0..16 {
            out.control_points[i][0] += self.control_points[i][0] - identity.control_points[i][0];
            out.control_points[i][1] += self.control_points[i][1] - identity.control_points[i][1];
        }
...
impl EmoteModelSchema {
    ...
    fn build_motion_scene_internal(
        ...
        let mut particle_emitters = if previous_scene.map_or(false, |scene| {
            time_ticks + f32::EPSILON >= scene.scene_time_ticks
        }) {
            previous_scene
...
fn native_manager_frame_order(a: &EmoteStaticSprite, b: &EmoteStaticSprite) -> std::cmp::Ordering {
...
fn finalize_scene(
    ...
    let (composite_mask_owners, composite_mask_sources_by_key, composite_referenced_type3) =
...
fn resolve_composite_mask_owners(
    ...
    for (owner_path, references) in raw {
        ...
        if owner.draw_frame_info.layer_type != 12 || (owner.draw_frame_info.stencil_type & 4) == 0 {
            continue;
        }
...
fn mark_native_draw_frame_submissions(
    ...
    for state in layer_states.iter_mut() {
        let submitted = match state.draw_frame_info.layer_type {
            0 | 10 | 12 => {
                state.visible
                    && emitted_sprite_keys.contains(&state.draw_frame_info.native_draw_key)
            }
            3 => {
                state.visible
...
impl Default for TravelContext {
    fn default() -> Self {
        Self {
            ...
            draw_index: 0,
            priority_ranks: Arc::new(BTreeMap::new()),
            layer_type: 0,
            mesh_transform: 0,
            mesh_combine: false,
            mesh_sync_child: 0,
...
fn parse_particle_motion_list(layer: &PsbValue) -> Vec<ParticleMotionRef> {
    let Some(items) = layer
        .field("particleMotionList")
        .and_then(PsbValue::as_list)
    else {
        return Vec::new();
    };
...
fn travel_layer(
    ...
    if let Some(frame_list) = layer.field("frameList").and_then(PsbValue::as_list) {
        let initial_state =
            evaluate_frame_list(frame_list, 0.0, None, ctx.coordinate.unwrap_or(0), None);
        ...
        for frame in frame_list {
            let Some(_frame_obj) = frame.as_object() else {
                continue;
            };
            let frame_value = frame;
            let Some(content) = frame_value
                .field("content")
                .filter(|content| content.as_object().is_some())
            else {
                continue;
            };
...
fn map_child_coordinate_through_source(source: InheritSourceState, delta: [f32; 3]) -> [f32; 3] {
...
fn apply_feedback_specialized_pass(
    ...
    let root_positions: BTreeMap<String, [f32; 3]> = layer_states[begin..end]
        ...
        .collect();
    let mut updated =
        BTreeMap::<String, (EmoteFeedbackRuntimeState, [f32; 6], f32, [u32; 4])>::new();
    ...
    for state in layer_states.iter_mut().take(end).skip(begin) {
        ...
        let active =
            state.visible && delta_ticks.abs() > f32::EPSILON && timespan.abs() > f32::EPSILON;
        let decay = if active {
        ...
        if !active {
            updated.insert(
                state.path.clone(),
                (runtime, state.transform, state.opacity, frame.colors),
            );
            continue;
        }
        ...
        state.opacity = state.opacity.max(1.0 / 255.0).powf(decay).clamp(0.0, 1.0);
        let neutral = if (frame.blend_mode & 0xF0) == 0x10 {
            128.0
        } else {
            255.0
        };
    ...
    for sprite in sprites.iter_mut().filter(|sprite| sprite.feedback_history) {
        if let Some((runtime, transform, opacity, colors)) =
            updated.get(&sprite.draw_frame_info.path)
        {
...
fn frame_easing(t: f32, easing_ref: Option<&PsbValue>, easing_table: Option<&[PsbValue]>) -> f32 {
...
fn evaluate_native_easing_piece(piece: &PsbValue, x: f32) -> Option<f32> {
    ...
    Some(linear + h * h * (cubic_u * ps[interval + 1] + cubic_v * ps[interval]) / 6.0)
}
...
fn evaluate_native_beziers_path(path: &PsbValue, t: f32) -> Option<[f32; 2]> {
    ...
    let base = segment * 3;
    Some([
        b0 * path_x[base] + b1 * path_x[base + 1] + b2 * path_x[base + 2] + b3 * path_x[base + 3],
        b0 * path_y[base] + b1 * path_y[base + 1] + b2 * path_y[base + 2] + b3 * path_y[base + 3],
    ])
}
...
fn interpolate_frame_content(
    ...
    if state.motion_direction_type == 3 {
        ...
        let tangent_coord_t0 = frame_easing(tangent_t0, current_content.field("ccc"), easing_table);
        let tangent_coord_t1 = frame_easing(tangent_t1, current_content.field("ccc"), easing_table);
    ...
    if state.single_color && next_state.single_color {
        ...
        for i in 0..4 {
            state.colors[i] =
                interpolate_native_packed_color(state.colors[i], next_state.colors[i], color_t);
        }
    }
...
fn merge_frame_content(state: &mut DynamicFrameState, content: &PsbValue) {
    ...
    if let Some(motion) = content.field("motion") {
        ...
        state.motion_direction_offset_complete =
            content_bool_like(motion, "docmpl").unwrap_or(false);
...
fn travel_layer_at<'a>(
    ...
    let native_mesh_chain_active = ctx.mesh_transform == 1 && (ctx.mesh_sync_child & 0x8) != 0;
    let native_mesh_chain_patch = if native_mesh_chain_active {
        draw_ctx.mesh_patch.filter(|patch| patch.domain.is_some())
    } else {
        None
    };
...
fn nested_motion_direction_degrees(
    ...
    match state.motion_direction_type {
        ...
        3 => state
            .motion_path_tangent_degrees
            .map(|angle| (angle + state.motion_direction_offset_degrees).rem_euclid(360.0)),
...
fn nested_motion_local_time(parent_local_time: f32, state: &DynamicFrameState) -> f32 {
    ...
    finite_or(
        parent_local_time - state.frame_start_ticks + state.time_offset_ticks,
        0.0,
    )
}
...
fn apply_ground_correction_specialized_pass(
    ...
    sprites: &mut [EmoteStaticSprite],
    layer_states: &mut [EmoteStepFrameLayerState],
    pending_nested: &mut [PendingNestedMotion<'_>],
) {
    ...
    for index in begin..end {
        ...
        let parent_raw = parent_path
            .as_deref()
            .and_then(|parent| {
                layer_states[begin..end]
                    .iter()
                    .find(|state| state.path == parent)
            })
            .map(|state| state.raw_position)
            .unwrap_or([0.0; 3]);
        ...
        let Some(corrected) = hook(&request) else {
            continue;
        };
...
fn apply_anchor_specialized_pass(
    ...
    for anchor in anchors {
        ...
        let Some(target) =
            find_scope_layer_position_with_root_fallback(layer_states, scope_start, &anchor.target)
        else {
            continue;
        };
...
fn apply_mesh_chain_specialized_pass(
    ...
    for state in layer_states.iter_mut().take(end).skip(scope_start) {
        ...
        for patch in state.mesh_chain.iter().rev() {
            let sync = MeshSyncChildState {
                patch: *patch,
                mask: 1,
                coordinate: Some(0),
            };
            if let Some(mapped) = mesh_sync_warp_point(sync, xy) {
                xy = mapped;
            }
        }
...
fn native_type7_local_bounds(state: &EmoteStepFrameLayerState) -> [f32; 4] {
    ...
    let transform = |x: f32, y: f32| -> [f32; 2] { [m[0] * x + m[1] * y, m[2] * x + m[3] * y] };
...
fn apply_ready_to_draw_specialized_pass(
    ...
    for state in layer_states[begin..end].iter_mut() {
        ...
        if special_drawable_type {
            ...
            if state.draw_frame_info.layer_type == 12 && state.draw_frame_info.stencil_wipe_enabled
            {
    ...
    for state in snapshot {
        ...
        let target = if camera.target.is_empty() {
            ...
            snapshot
                .iter()
                .find(|candidate| {
                    candidate.draw_frame_info.layer_label.as_deref() == Some(camera.target.as_str())
...
fn apply_camera_specialized_pass(
    ...
    let end = scope_end.min(layer_states.len());
    let snapshot: Vec<&EmoteStepFrameLayerState> =
        layer_states[scope_start.min(end)..end].iter().collect();
...
fn apply_type7_bounds_specialized_pass(
    ...
    for state in layer_states.iter_mut().take(end).skip(scope_start) {
        ...
        let rect = if state.draw_frame_info.layer_type == 7 && state.visible {
            ...
            inherited
                .and_then(|p| intersect_clip_rect(own, p))
                .or(Some(own).filter(|_| inherited.is_none()))
        } else {
            inherited
        };
...
fn particle_motion_timing(
    ...
    Some((
        last,
        loop_time.is_finite() && loop_time >= 0.0 && loop_time < last,
    ))
}

fn particle_instance_finished(object_table: &PsbValue, particle: &ParticleInstanceRuntime) -> bool {
    let Some((last, looped)) =
        particle_motion_timing(object_table, &particle.object_name, &particle.motion_name)
    else {
        return true;
    };
...
fn spawn_particle_instance(
    ...
    match config.particle {
        1 => {
            let azimuth = particle_random_range(&mut runtime.rng_state, 0.0, std::f32::consts::TAU);
            if config.tri_volume {
                let polar =
                    particle_random_range(&mut runtime.rng_state, 0.0, std::f32::consts::TAU);
    ...
    Some(ParticleInstanceRuntime {
        ...
        angle_degrees,
        zoom,
        opacity: if config.inherit_opacity != 0 {
            emitter.opacity
        } else {
            1.0
        },
...
fn particle_spawn_count(
    ...
    while runtime.countdown_ticks <= 0.0 && guard < 4096 {
        ...
        let lo = 60.0 / frame.fmin.max(f32::EPSILON);
        let hi = 60.0 / frame.fmax.max(f32::EPSILON);
        let interval = particle_random_range(&mut runtime.rng_state, lo, hi)
            .abs()
            .max(f32::EPSILON);
...
fn apply_particle_specialized_pass(
    ...
    while cursor < layer_states.len() {
        ...
        let Some(frame) = emitter
            .specialized_frame
            .as_ref()
            .and_then(|state| state.particle)
        else {
            continue;
        };
        let angle = particle_emitter_angle_degrees(emitter.transform);
        let runtime = emitters.entry(emitter.path.clone()).or_insert_with(|| {
            ParticleEmitterRuntime::new(
                &emitter.path,
...
fn find_scope_layer_position(states: &[ScopeLayerPosition], target: &str) -> Option<[f32; 3]> {
...
fn resolve_pending_nested_motions(
    pending: Vec<PendingNestedMotion<'_>>,
...
fn recurse_motion_at(
    ...
    apply_type7_bounds_specialized_pass(
        layer_states,
        scope_start,
        scope_end,
        out,
...
mod tests {
    ...
    fn test_runtime_sprite(path: &str, native_key: &[u64], label: &str) -> EmoteStaticSprite {
        let ctx = TravelContext {
            path: path.to_owned(),
            native_draw_key: native_key.to_vec(),
            layer_type: 0,
    ...
    fn native_manager_sort_uses_exact_z_before_priority_emission_key() {
        ...
        assert_eq!(
            native_manager_frame_order(&back, &front),
            std::cmp::Ordering::Less
        );
        assert_eq!(
            native_manager_frame_order(&front, &back),
    ...
    fn ready_drawable_parent_exposes_native_dfi_parent_key() {
        ...
        apply_ready_to_draw_specialized_pass(&mut states, 0, 2, &mut sprites, 0);
        assert_eq!(
            states[1].draw_frame_info.stencil_parent_path.as_deref(),
            Some("mask")
        );
        assert_eq!(
            states[1].draw_frame_info.stencil_parent_native_key,
            Some(vec![7])
        );
    }
    ...
    fn nested_player_appends_its_priority_rank_to_parent_slot() {
        ...
        let child_ranks = Arc::new(child_ranks);
        let child = enter_layer_context(
            TravelContext {
                native_draw_prefix: vec![5],
                priority_ranks: child_ranks,
                ..TravelContext::default()
            },
    ...
    fn test_mesh_patch(dx: f32, dy: f32) -> EmoteMeshPatch {
    ...
    fn native_mesh_combine_collapses_inclusive_noncombining_active_parent() {
        ...
        advance_native_mesh_combine_chain(&mut chain, &mut candidate, 0, Some(parent), true, false);
        ...
        assert!(
            (chain[0].control_points[5][0] - (identity.control_points[5][0] + 0.40)).abs() < 1.0e-6
        );
        assert!(
            (chain[0].control_points[5][1] - (identity.control_points[5][1] + 0.10)).abs() < 1.0e-6
        );
    }
    ...
    fn native_mesh_combine_inactive_false_parent_is_a_hard_barrier() {
        ...
        advance_native_mesh_combine_chain(&mut chain, &mut candidate, 0, None, false, false);
    ...
    fn native_mesh_combine_inactive_true_parent_preserves_parent_walk() {
        ...
        advance_native_mesh_combine_chain(&mut chain, &mut candidate, 0, None, false, true);
        assert_eq!(candidate, 0);
    ...
    fn native_combine_mesh_adds_identity_relative_displacements() {
        ...
        assert!(
            (combined.control_points[5][0] - (identity.control_points[5][0] - 0.20)).abs() < 1.0e-6
        );
        assert!(
            (combined.control_points[5][1] - (identity.control_points[5][1] + 0.20)).abs() < 1.0e-6
        );
    }
    ...
    fn native_combine_mesh_is_not_bezier_function_composition() {
        ...
        assert!(
            (native.control_points[5][0] - composed[0]).abs() > 1.0e-4
    ...
    fn native_frame_easing_matches_recovered_cubic_spline_formula() {
        ...
        let u = x;
        let v = 1.0 - u;
        let expected = v * 0.0 + u * 1.0 + ((u * u * u - u) * -1.0 + (v * v * v - v) * 2.0) / 6.0;
    ...
    fn unparameterized_layer_uses_motion_player_time() {
        ...
        let eval = layer_parameter_eval(&layer, None, &BTreeMap::new(), &frame_list, 37.5).unwrap();
    ...
    fn parameterized_layer_uses_native_range_and_division_mapping() {
        ...
        let frame_list = vec![test_frame(0.0, 1, test_content(vec![]))];
        let eval =
            layer_parameter_eval(&layer, Some(&parameters), &variables, &frame_list, 0.0).unwrap();
    ...
    fn native_opacity_interpolation_rounds_to_nearest_byte() {
        let frames = vec![
            test_frame(0.0, 3, test_content(vec![("opa", PsbValue::Int(0))])),
            test_frame(10.0, 1, test_content(vec![("opa", PsbValue::Int(255))])),
        ];
        let state = evaluate_frame_list(&frames, 5.0, None, 0, None);
    ...
    fn native_cp_maps_xy_plane_and_linearly_interpolates_z() {
        ...
        let result =
            interpolate_native_coordinate([0.0, 0.0, 0.0], [10.0, 0.0, 4.0], 0.5, 0, Some(&path));
    ...
    fn native_cp_maps_xz_plane_and_linearly_interpolates_y() {
        let path = test_inline_cp_path();
        let result =
            interpolate_native_coordinate([0.0, 0.0, 0.0], [10.0, 3.0, 0.0], 0.5, 1, Some(&path));
        assert!((result[0] - 5.0).abs() < 1.0e-6);
    ...
    fn native_packed_color_uses_integer_lane_interpolation() {
        assert_eq!(
            interpolate_native_packed_color(0x0000_0000, 0xFFFF_FFFF, 0.0),
            0x0000_0000
        );
        assert_eq!(
            interpolate_native_packed_color(0x0000_0000, 0xFFFF_FFFF, 0.5),
            0x7F7F_7F7F
        );
        assert_eq!(
            interpolate_native_packed_color(0x0000_0000, 0xFFFF_FFFF, 1.0),
            0xFFFF_FFFF
        );
        assert_eq!(
            interpolate_native_packed_color(0x1234_5678, 0x1234_5678, 0.25),
            0x1234_5678
        );
    }
    ...
    fn particle_trigger_dirty_tracks_serialized_frame_switch_not_interpolation() {
        ...
        same_key.opa = 42.0;
        assert!(!serialized_frame_transition_dirty(
            Some(&previous),
            &same_key
        ));
    ...
    fn particle_tri_volume_scale_matches_native_sqrt_abs_determinant() {
        ...
        assert!(
            (particle_transform_scale([-2.0, 0.0, 0.0, 3.0, 0.0, 0.0]) - 6.0f32.sqrt()).abs()
                < 1.0e-6
        );
    }
