...
- 编译 ASB/IET 直接解析二进制记录，保留 Lua 字符串、源行号和编译器跳转地址，修复含内嵌引号的脚本在启动时解析失败，以及编译条件分支和循环错误。
- 编译宏保留原文件标签、条件返回和调用期间的参数作用域，支持动态调用目标，防止按钮、语音等宏的可选参数相互污染。
- 按接口约定让 `e:var` 返回字符串、标签过滤器接收求值后的参数，修复非数字字符串比较错误，并补充宏局部变量的查询、写入和恢复。
- 补充 `getFrameNumber`、`getScriptSize`，图层查询返回可见性、透明度及变换属性，修复另一类标签宏模板中按钮未注册、开始游戏后脚本停止的问题。
